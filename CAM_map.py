...
def main_worker(local_rank, nprocs, args):
    ...
    model.eval()
    # Grad-CAM 
    target_layers = [model.module.decoder.final_up[-1]]
    ...
    with torch.enable_grad():
        for sample in test_loader:
            ...
            cam_sum = torch.zeros(batch_size, 1, H, W, device=device)
            ...
            for y in range(0, H - PATCH + 1, STRIDE):
                for x in range(0, W - PATCH + 1, STRIDE):
                    ...
                    pred = model(patch)
                    pred = pred.detach() * window

                    grayscale_cam = cam(input_tensor=patch, targets=[NormalMapTarget()] * batch_size)
                    cam_tensor = torch.from_numpy(grayscale_cam).unsqueeze(1).to(device)
                    cam_tensor = cam_tensor * window
                    out_sum[..., y:y + PATCH, x:x + PATCH] += pred
                    cam_sum[..., y:y + PATCH, x:x + PATCH] += cam_tensor
            ...
            # ----------- Grad-CAM Map -----------
            full_cam = cam_sum / w_sum.clamp_min(1e-6)
            ...
            for b in range(batch_size):
                ...
                b_cam = full_cam[b, 0].cpu().numpy()
                b_cam[b_mask.squeeze().cpu().numpy() == 0] = 0 # remove invalid pixels (optional)
                b_cam = (b_cam - b_cam.min()) / (b_cam.max() - b_cam.min() + 1e-8)
                b_cam_color = cv2.applyColorMap((b_cam * 255).astype(np.uint8), cv2.COLORMAP_JET)
                cv2.imwrite(f'{args.error_maps_dir}/{b_filename}_gradcam.png', b_cam_color)

                # Overlay
                base = image[b, 0].cpu().numpy()
                base = (base - base.min()) / (base.max() - base.min() + 1e-8)
                base_rgb = cv2.cvtColor((base * 255).astype(np.uint8), cv2.COLOR_GRAY2BGR)
                overlay = cv2.addWeighted(base_rgb, 0.5, b_cam_color, 0.5, 0)
                cv2.imwrite(f'{args.error_maps_dir}/{b_filename}_gradcam_overlay.png', overlay)
